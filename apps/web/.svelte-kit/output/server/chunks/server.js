...
function update_derived_status(derived) {
  if ((derived.f & 512) !== 0 || derived.deps === null)
...
var Batch = class Batch {
  ...
  #process() {
    ...
    if (this.#roots.length > 0) {
      ...
      batch.#roots.push(
        ...this.#roots.filter((r) => !batch.#roots.includes(r)),
      );
    }
    if (next_batch !== null) {
  ...
  #traverse(root, effects, render_effects) {
    ...
    while (effect !== null) {
      ...
      if (
        ...
        else if (async_mode_flag && (flags & 16777224) !== 0)
...
function set(source, value, should_proxy = false) {
  ...
  return internal_set(
    source,
    should_proxy ? proxy(value) : value,
    legacy_updates,
  );
}
/**
  ...
  return new Proxy(value, {
    ...
    deleteProperty(target, prop) {
      ...
      if (s === void 0) {
        if (prop in target) {
          const s = with_parent(() =>
            /* @__PURE__ */ state(UNINITIALIZED, stack),
          );
          sources.set(prop, s);
          increment(version);
        }
...
function destroy_effect(effect, remove_dom = true) {
  ...
  if (transitions !== null)
    for (const transition of transitions) transition.stop();
...
function is_dirty(reaction) {
  ...
  if ((flags & 4096) !== 0) {
    ...
    if ((flags & 512) !== 0 && batch_values === null)
...
async function sha256(data) {
  ...
  return base64_encode(
    await crypto.subtle.digest("SHA-256", text_encoder.encode(data)),
  );
}
/**
...
//#endregion
export {
  experimental_async_required as $,
  component_root as A,
  ...
  derived$1 as S,
  active_effect as T,
  async_mode_flag as U,
  pop$1 as V,
  hydrate_node as W,
  state_proxy_unmount as X,
  lifecycle_double_unmount as Y,
  HYDRATION_ERROR as Z,
  lifecycle_function_unavailable as _,
  store_get as a,
  run as at,
  escape_html as b,
  get_render_context as c,
  getContext as d,
  LEGACY_PROPS as et,
  hasContext as f,
  hydratable_serialization_failed as g,
  ...
  set_hydrating as q,
  render as r,
  define_property as rt,
  get_user_code_location as s,
  derived as t,
  STATE_SYMBOL as tt,
  getAllContexts as u,
  getAbortSignal as v,
