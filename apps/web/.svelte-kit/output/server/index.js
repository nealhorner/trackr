import {
  a as set_manifest,
  i as read_implementation,
  n as options,
  o as set_read_implementation,
  t as get_hooks,
...
import {
  C as MUTATIVE_METHODS,
  S as ENDPOINT_METHODS,
  _ as deserialize_binary_form,
  a as handle_error_and_jsonify,
  c as method_not_allowed,
  d as static_error_page,
  f as escape_html,
  i as get_node_type,
  l as redirect_response,
  m as negotiate,
  n as format_server_error,
  o as handle_fatal_error,
  p as is_form_content_type,
  r as get_global_name,
  s as has_prerendered_path,
...
import {
  a as stringify,
  d as get_status,
  f as normalize_error,
  g as text_encoder,
  h as text_decoder,
  i as parse_remote_arg,
  m as get_relative_path,
...
import {
  _ as has_data_suffix,
  b as strip_resolution_suffix,
  d as make_trackable,
  f as normalize_path,
  g as add_resolution_suffix,
  h as add_data_suffix,
  i as validate_page_server_exports,
  l as decode_pathname,
  m as noop_span,
  ...
  u as disable_search,
  v as has_resolution_suffix,
  x as compact,
  y as strip_data_suffix,
...
function stringify_action_response(data, route_id, transport) {
  ...
  return try_serialize(
    data,
    (value) => devalue.stringify(value, encoders),
    route_id,
  );
}
/**
...
function server_data_serializer_json(event, event_state, options) {
  ...
  const reducers = {
    ...Object.fromEntries(
      Object.entries(options.hooks.transport).map(([key, value]) => [
        key,
        value.encode,
      ]),
    ),
  ...
  return {
    add_node(i, node) {
      try {
        ...
        strings[i] =
          `{"type":"data","data":${devalue.stringify(node.data, reducers)},"uses":${JSON.stringify(serialize_uses(node))}${node.slash ? `,"slash":${JSON.stringify(node.slash)}` : ""}}`;
      } catch (e) {
        e.path = "data" + e.path;
        throw new Error(clarify_devalue_error(event, e), { cause: e });
      }
...
function create_universal_fetch(event, state, fetched, csr, resolve_opts) {
  ...
  const universal_fetch = async (input, init) => {
    ...
    if (same_origin) {
      ...
      if (
        (input instanceof Request ? input.mode : (init?.mode ?? "cors")) ===
        "no-cors"
      )
    ...
    try {
      ...
      rendered = await with_request_store(
        ...
        async () => {
          ...
          const { head, html, css, hashes } = options.async
            ? await rendered
            : rendered;
...
async function handle_remote_call_internal(
  ...
  async function serialize_refreshes(client_refreshes) {
    ...
    return stringify(
      Object.fromEntries(
        await Promise.all(
          Object.entries(refreshes).map(async ([key, promise]) => [
            key,
            await promise,
          ]),
        ),
      ...
      fetch: async (info, init) => {
        ...
        if (set_cookie)
          for (const str of set_cookie_parser.splitCookiesString(set_cookie)) {
            const { name, value, ...options } = set_cookie_parser.parseString(
              str,
              { decodeValues: false },
            );
            set_internal(name, value, {
...
var page_methods = new Set(["GET", "HEAD", "POST"]);
var allowed_page_methods = new Set(["GET", "HEAD", "OPTIONS"]);
  ...
  const remote_id = get_remote_id(url);
  {
    ...
    if (remote_id) {
      ...
      if (
        ...
        (!request_origin ||
          !options.csrf_trusted_origins.includes(request_origin))
      ) {
                ...
                return record_span({
                  ...
                  fn: (resolve_span) => {
                    return with_request_store(null, () =>
                      resolve(
                        merge_tracing(event, resolve_span),
                        page_nodes,
                        opts,
                      ).then((response) => {
          ...
          if (!page_nodes)
            ...
            if (method === "OPTIONS")
              ...
              response = method_not_allowed(
                [...allowed_methods].reduce((acc, curr) => {
                  acc[curr] = true;
                  return acc;
                }, {}),
                method,
