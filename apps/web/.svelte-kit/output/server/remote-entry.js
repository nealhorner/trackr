...
import {
  ...
  b as set_nested_value,
  g as deep_set,
  h as create_field_proxy,
  v as flatten_issues,
  y as normalize_issue,
} from "./chunks/utils.js";
import {
  a as stringify,
  o as stringify_remote_arg,
...
function command(validate_or_fn, maybe_fn) {
  ...
  const wrapper = (arg) => {
    ...
    if (!state.allows_commands) {
      const disallowed_method = !MUTATIVE_METHODS.includes(
...
function form(validate_or_fn, maybe_fn) {
  ...
  const schema =
    !maybe_fn || validate_or_fn === "unchecked" ? null : validate_or_fn;
  /**
    ...
    const __ = {
      ...
      fn: async (data, meta, form_data) => {
        ...
        if (meta.validate_only)
          return (
            validated?.issues?.map((issue) => normalize_issue(issue, true)) ??
            []
          );
    ...
    if (key == void 0)
      Object.defineProperty(instance, "for", {
        value: (key) => {
          ...
          let instance = (state.form_instances ??=
            /* @__PURE__ */ new Map()).get(cache_key);
          if (!instance) {
        ...
        try {
          return await get_response(__, arg, state, async () => {
            ...
            return parse_remote_response(
              await (cache[key] ??= fetch(
                new URL(url, event.url.origin).href,
              ).then(async (response) => {
                if (!response.ok)
...
function query(validate_or_fn, maybe_fn) {
  ...
  const wrapper = (arg) => {
    ...
    promise.catch(() => {});
    promise.set = (value) =>
      update_refresh_value(get_refresh_context(__, "set", arg), value);
    promise.refresh = () => {
      const refresh_context = get_refresh_context(__, "refresh", arg);
      const is_immediate_refresh =
        !refresh_context.cache[refresh_context.cache_key];
      return update_refresh_value(
        refresh_context,
        is_immediate_refresh ? promise : get_remote_function_result(),
        is_immediate_refresh,
      );
    };
